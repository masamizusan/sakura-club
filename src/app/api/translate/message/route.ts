...
const SUPPORTED_LANGUAGES = ['ja', 'en', 'ko', 'zh-tw'] as const
...
async function translateWithOpenAI(text: string, targetLang: SupportedLanguage, sourceLang?: string): Promise<string> {
  const apiKey = process.env.OPENAI_API_KEY
  if (!apiKey) {
    console.error('[TRANSLATE_MSG] OPENAI_API_KEY is not configured')
  ...
  const response = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: 'system',
          content: `You are a professional translator for a dating/social app. Translate the following message into ${targetLanguageName}.
...
5. If the text is already in ${targetLanguageName}, return it as-is`
  ...
  if (!response.ok) {
    ...
    console.error('[TRANSLATE_MSG] OpenAI API error:', response.status, errorText)
  ...
  if (!translated) {
    console.error('[TRANSLATE_MSG] OpenAI returned empty translation')
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (authError) {
      console.error('[TRANSLATE_MSG] Auth error:', authError.message)
    }
    if (!user) {
    ...
    if (!checkRateLimit(user.id)) {
      console.warn('[TRANSLATE_MSG] Rate limit exceeded for user:', user.id.slice(0, 8))
      return NextResponse.json(
    ...
    if (!messageId || !text || !targetLanguage) {
      return NextResponse.json(
        { error: 'Missing required fields: messageId, text, targetLanguage', code: ERROR_CODES.INVALID_PARAMS },
        { status: 400 }
      )
    }
    ...
    if (text.length > 1000) {
      return NextResponse.json(
        { error: 'Text too long (max 1000 characters)', code: ERROR_CODES.TEXT_TOO_LONG },
        { status: 400 }
      )
    }
    ...
    if (cached) {
      console.log('[TRANSLATE_MSG] Cache hit for message:', messageId.slice(0, 8), 'lang:', targetLanguage)
      return NextResponse.json({
    ...
    try {
      translatedText = await translateWithOpenAI(text, targetLanguage as SupportedLanguage)
    } catch (translateError) {
      const errMsg = translateError instanceof Error ? translateError.message : 'Unknown error'
      console.error('[TRANSLATE_MSG] Translation failed:', errMsg)
    ...
    console.log('[TRANSLATE_MSG] Success for message:', messageId.slice(0, 8), 'lang:', targetLanguage, 'duration:', duration, 'ms')
    ...
    console.error('[TRANSLATE_MSG] Unexpected error:', error)
    return NextResponse.json(
      { error: 'Translation failed', code: ERROR_CODES.UNKNOWN },
      { status: 500 }
    )

...
### 既読機能（チャットの ✓✓ 表示） — 2026/04/26 非実装確定
引き継ぎ書には「次の作業」として記載されていたが、
プロダクトオーナー判断で非実装を確定。

理由:
- 初対面・関係性のないユーザー間で既読プレッシャーは離脱を生む
- LINEは関係性ある相手の安心感だが、マッチングアプリでは
  過剰な拒絶解釈を生む（既読スルー文化）
- 日本人女性（無料）の気軽さを損なう
- 外国人男性（有料）の自己反省・解約リスク
- 主要マッチングアプリ業界（Tinder/Bumble/Pairs/with）も
  既読非搭載が主流
- ブランド軸「静謐・本物の日本＝ゆっくり対話する場」と整合
